...
<step number="2" name="research_tech_stack_libraries">

### Step 2: Research Tech Stack Libraries and Verify Versions (Conditional)

If tech stack includes specific libraries or frameworks, use Context7 MCP to fetch current best practices, patterns, and verify latest stable versions.

<when_to_use>
  IF user specified tech stack includes:
    - Specific frameworks (Next.js, NestJS, etc.)
    - UI libraries (React, shadcn/ui, etc.)
    - Specialized tools (Prisma, TypeORM, etc.)
  THEN:
    Fetch documentation via Context7 MCP for architecture patterns and version info
  ELSE:
    SKIP this step
</when_to_use>

<library_identification>
  FROM user input in Step 1:
    - Extract main framework choices
    - Identify key libraries that will shape architecture
    - Focus on 1-3 core technologies
</library_identification>

<instructions>
  ACTION: Identify if core framework documentation is needed
  IF framework patterns needed:
    1. Use mcp__mcp-server-context7__resolve-library-id
       - Search for main framework (e.g., "Next.js", "NestJS")
       - Note returned library information for latest version
    2. Use mcp__mcp-server-context7__get-library-docs
       - Provide Context7-compatible library ID
       - DO NOT specify version in libraryID to get latest stable version
       - Topic: "project structure" or "best practices" or "getting started"
       - Extract current stable version from documentation
    3. Update tech stack references with latest stable versions
    4. Use insights to inform mission and roadmap
  ELSE:
    Proceed to next step
</instructions>

<version_extraction>
  FROM Context7 documentation response:
    - Look for version numbers in library metadata
    - Check "Getting Started" or "Installation" sections for version info
    - Note breaking changes or migration guides if upgrading
    - Prefer stable releases over canary/beta versions
</version_extraction>
...
<examples>
  <example_1>
    ...
    → get-library-docs: libraryID="/vercel/next.js", topic="getting started"
    → Extract version: "15.1.0" (latest stable)
    → Update tech-stack.md: "Next.js 15.1.0"
  </example_1>
  <example_2>
    Tech Stack: NestJS + PostgreSQL
    → resolve-library-id: "nestjs"
    → get-library-docs: libraryID="/nestjs/nest", topic="getting started"
    → Extract version: "10.3.0" (latest stable)
    → Update tech-stack.md: "NestJS 10.3.0"
  </example_2>
  <example_3>
    Tech Stack: React + TailwindCSS
    → resolve-library-id: "tailwindcss"
    → get-library-docs: libraryID="/tailwindlabs/tailwindcss", topic="installation"
    → Extract version: "4.0.1" (latest stable)
    → Update tech-stack.md: "TailwindCSS 4.0.1"
  </example_3>
</examples>

</step>

<step number="3" subagent="file-creator" name="create_documentation_structure">
...
<step number="4" subagent="file-creator" name="create_mission_md">
...
<step number="5" subagent="file-creator" name="create_tech_stack_md">
...
<step number="6" subagent="file-creator" name="create_mission_lite_md">
...
<step number="7" subagent="file-creator" name="create_roadmap_md">
