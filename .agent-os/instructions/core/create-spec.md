...
<step number="8" name="library_documentation_research">

### Step 8: Research Library Documentation and Verify Versions (Conditional)

If the spec requires new libraries or frameworks, use Context7 MCP to fetch up-to-date documentation, best practices, and latest stable versions.

<when_to_use>
  IF spec involves:
    - New external dependencies listed in tech requirements
    - Unfamiliar library or framework integration
    - Specific library patterns needed for implementation
  THEN:
...
<library_identification>
  FROM user requirements and tech-stack.md:
    - Identify new libraries/frameworks needed
    - Check if library is already in tech-stack.md
    - Determine what documentation is needed
</library_identification>

<instructions>
  ACTION: Identify if new libraries are needed
  IF new libraries required:
    1. Use mcp__mcp-server-context7__resolve-library-id
       - Search for library name
       - Note returned library information
    2. Use mcp__mcp-server-context7__get-library-docs
       ...
       - Specify relevant topic for this spec (e.g., "getting started", specific feature)
       - Extract latest stable version from documentation
       - Review documentation for implementation patterns
    3. Document in technical-spec.md:
       - Library name and latest stable version
...
<version_extraction>
  FROM Context7 documentation:
    - Extract current stable version number
    - Note installation commands with version
...
<examples>
  <example_1>
    ...
    → get-library-docs: libraryID="/stripe/stripe-node", topic="checkout session"
    → Extract version: "14.10.0"
    → Document: "stripe": "^14.10.0" with checkout session patterns
  </example_1>
  <example_2>
    Spec: Real-time notifications
    → resolve-library-id: "socket.io"
    → get-library-docs: libraryID="/socketio/socket.io", topic="rooms and namespaces"
    → Extract version: "4.7.2"
    → Document: "socket.io": "^4.7.2" with room management patterns
  </example_2>
  <example_3>
    Spec: Form validation
    → resolve-library-id: "zod"
    → get-library-docs: libraryID="/colinhacks/zod", topic="getting started"
    → Extract version: "3.22.4"
    → Document: "zod": "^3.22.4" with schema validation patterns
  </example_3>
</examples>
